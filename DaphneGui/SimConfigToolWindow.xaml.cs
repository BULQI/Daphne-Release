...
namespace DaphneGui
{
	...
	{                
		...
		private void CellsListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
            ...
            DivExpander_Expanded(null, null);
		}
        ...
        private void DiffSchemeExpander_Expanded(object sender, RoutedEventArgs e)
        {
            ...
            DiffGridPopulate(cell, er);
        }

		private void DiffGridPopulate(ConfigCell cell, EntityRepository er)
		{
			//Clear the grids
		...
		private void menuDeleteStates_Click(object sender, RoutedEventArgs e)
		{
            ...
            if (cell.diff_scheme.Driver.states.Count < 3)
            {
                MessageBox.Show("Differentiation scheme must have at least two states.");
                return;
            }
			...
			foreach (ConfigActivationRow diffrow in diff_scheme.activationRows.ToList())
			{
				if (EpigeneticMapGrid.SelectedItems.Contains(diffrow))
				{
					...
					//this removes the activation row and transition driver info for state from the differentiation scheme
					diff_scheme.RemoveActivationRow(diffrow);

                    DiffGridPopulate(cell, MainWindow.SC.SimConfig.entity_repository);
				}
				i++;
			}
        ...
        /// <summary>
        /// This method is called when the user clicks on a row in the division grid.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DivRegGrid_SelectedCellsChanged(object sender, SelectedCellsChangedEventArgs e)
        {
            var selectedRow = DivRegGrid.GetSelectedRow();
            if (selectedRow == null)
                return;

            int row = DivRegGrid.SelectedIndex;

            DataGridCellInfo selected = DivRegGrid.SelectedCells[0];
            DataGridColumn col = selected.Column;
        }
        //private void btnDelDivScheme_Click(object sender, RoutedEventArgs e)
        //{
        //    MessageBoxResult res;
        //    res = MessageBox.Show("Are you sure you want to delete the selected cell's division scheme?", "Warning", MessageBoxButton.YesNo);
        //    if (res == MessageBoxResult.No)
        //        return;
        ...
        //    //Clear the grids
        //    DivRegGrid.ItemsSource = null;
        //    DivRegGrid.Columns.Clear();
        //}
        ...
        private void btnNewDivDriver_Click(object sender, RoutedEventArgs e)
        {
            ...
            // create a 2-state driver
            // can't generically assign molecules, because we don't now what molecules and reactions are in the cell
            cell.div_driver = new ConfigTransitionDriver();
            cell.div_driver.Name = "two-state";
            AddDivState("quiescent");
            AddDivState("mitotic");
            cell.div_driver.CurrentState = 0;
            cell.div_driver.StateName = cell.div_driver.states[cell.div_driver.CurrentState];
        }
        ...
        private void DivExpander_Expanded(object sender, RoutedEventArgs e)
        {
            ...
            DivGridPopulate(cell, er);
        }

        /// <summary>
        /// This method gets called after the DivRegGrid gui objects are generated. 
        /// This is the place to disable the diagonal grid cells.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DivRegItemContainerGenerator_StatusChanged(object sender, EventArgs e)
        {
            if (DivRegGrid.ItemContainerGenerator.Status == GeneratorStatus.ContainersGenerated)
            {
                DivRegGrid.ItemContainerGenerator.StatusChanged -= DivRegItemContainerGenerator_StatusChanged;
                int nRows = DivRegGrid.Items.Count;
                for (int j = 0; j < nRows; j++)
                {
                    var currRow = DivRegGrid.GetRow(j);
                    int nCols = DivRegGrid.Columns.Count;
                    for (int i = 0; i < nCols; i++)
                    {
                        if (j == i)
                        {
                            var columnCell = DivRegGrid.GetCell(currRow, i);
                            if (columnCell != null)
                            {
                                columnCell.IsEnabled = false;
                                columnCell.Background = Brushes.LightGray;
            ...
            //Generate the row headers
            DivRegGenerateRowHeaders();
        }

        private void DivRegGenerateRowHeaders()
        {
            //The code below generates the row headers
            ConfigCell cell = CellsListBox.SelectedItem as ConfigCell;
            if (cell == null)
            ...
            int rowcount = DivRegGrid.Items.Count;
            for (int ii = 0; ii < rowcount; ii++)
            {
                if (ii >= cell.div_driver.states.Count)
                    break;

                DataGridRow row = DivRegGrid.GetRow(ii);
                if (row != null)
                {
                    string sbind = string.Format("SelectedItem.div_driver.states[{0}]", ii);
                    Binding b = new Binding(sbind);
                    b.ElementName = "CellsListBox";
                    b.Path = new PropertyPath(sbind);
                    b.Mode = BindingMode.OneWay;
        ...
        private DataGridTemplateColumn CreateDivRegColumn(EntityRepository er, ConfigCell cell, string state)
        {
            DataGridTemplateColumn col = new DataGridTemplateColumn();

            string sbind = string.Format("SelectedItem.div_driver.states[{0}]", DivRegGrid.Columns.Count);
            Binding bcol = new Binding(sbind);
            bcol.ElementName = "CellsListBox";
            bcol.Path = new PropertyPath(sbind);
            bcol.Mode = BindingMode.OneWay;
            ...
            //SET UP A TEXTBLOCK ONLY
            Binding bn = new Binding(string.Format("elements[{0}].driver_mol_guid_ref", DivRegGrid.Columns.Count));
            bn.Mode = BindingMode.TwoWay;
            bn.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            MolGUIDtoMolNameConverter c = new MolGUIDtoMolNameConverter();
            bn.Converter = c;
            ...
            //NEED TO SOMEHOW CONVERT driver_mol_guid_ref to mol_pop!  Set up a converter and pass it the cytosol.
            MolGuidToMolPopForDiffConverter conv2 = new MolGuidToMolPopForDiffConverter();
            string sText = string.Format("elements[{0}].driver_mol_guid_ref", DivRegGrid.Columns.Count);
            Binding b3 = new Binding(sText);
            b3.Mode = BindingMode.TwoWay;
            b3.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            b3.Converter = conv2;
            ...
            //This disables the expander if no driver molecule is selected
            DriverElementToBoolConverter enabledConv = new DriverElementToBoolConverter();
            Binding bEnabled = new Binding(string.Format("elements[{0}].driver_mol_guid_ref", DivRegGrid.Columns.Count));
            bEnabled.Mode = BindingMode.OneWay;
            bEnabled.Converter = enabledConv;
            ...
            //SET UP THE ALPHA TEXTBOX
            Binding b = new Binding(string.Format("elements[{0}].Alpha", DivRegGrid.Columns.Count));
            b.Mode = BindingMode.TwoWay;
            b.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            FrameworkElementFactory txtDriverAlpha = new FrameworkElementFactory(typeof(TextBox));
            txtDriverAlpha.SetBinding(TextBox.TextProperty, b);
            ...
            //SET UP THE BETA TEXTBOX
            Binding beta = new Binding(string.Format("elements[{0}].Beta", DivRegGrid.Columns.Count));
            beta.Mode = BindingMode.TwoWay;
            beta.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            FrameworkElementFactory txtDriverBeta = new FrameworkElementFactory(typeof(TextBox));
            txtDriverBeta.SetBinding(TextBox.TextProperty, beta);
        ...
        /// <summary>
        /// This method adds a division state given a name 
        /// </summary>
        /// <param name="name"></param>
        private void AddDivState(string name)
        {
            EntityRepository er = MainWindow.SC.SimConfig.entity_repository;
            ConfigCell cell = CellsListBox.SelectedItem as ConfigCell;
            ...
            //If no div driver defined for this cell, return? Shouldn't happen?
            if (cell.div_driver == null)
            {
                return;
            }

            cell.div_driver.AddState(name);
            DivRegGrid.Columns.Add(CreateDivRegColumn(er, cell, name));
            DivRegGrid.ItemsSource = null;
            DivRegGrid.ItemsSource = cell.div_driver.DriverElements;
            UpdateDivRegGrid();
        }

        /// <summary>
        /// This method updates the division regulators grid.
        /// It is meant to be called after the user adds a new div state.
        /// </summary>
        private void UpdateDivRegGrid()
        {
            if (DivRegGrid.ItemContainerGenerator.Status == GeneratorStatus.ContainersGenerated)
            {
                int nRows = DivRegGrid.Items.Count;
        ...
        /// <summary>
        /// This method is called when the user clicks on Add State menu item for division driver
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void menuAddDivState_Click(object sender, RoutedEventArgs e)
        {
            //Show a dialog that gets the new state's name
            AddDiffState ads = new AddDiffState();
            ...
            if (ads.ShowDialog() == true)
            {
                AddDivState(ads.StateName);
            }
        }

        private void menuDeleteDivState_Click(object sender, RoutedEventArgs e)
        {
            EntityRepository er = MainWindow.SC.SimConfig.entity_repository;
            ConfigCell cell = CellsListBox.SelectedItem as ConfigCell;
            ...
            if (cell.div_driver.states.Count < 3)
            {
                MessageBox.Show("Division scheme must have at least two states.");
                return;
            }

            int i = 0;
            foreach (ConfigTransitionDriverRow divrow in cell.div_driver.DriverElements.ToList())
            {
                if (DivRegGrid.SelectedItems.Contains(divrow))
                {
                    int index = cell.div_driver.DriverElements.IndexOf(divrow);
                    string stateToDelete = cell.div_driver.states[index];

                    //this deletes the column from the division regulators grid
                    DeleteDivRegGridColumn(stateToDelete);
                    // this deletes the appropriate data in the ConfigTransitionDriver
                    cell.div_driver.DeleteState(index);
                    // this redraws the data grid
                    DivGridPopulate(cell, MainWindow.SC.SimConfig.entity_repository);
                }
                i++;
            }
        ...
        private void DeleteDivRegGridColumn(string state)
        {
            foreach (DataGridTemplateColumn col in DivRegGrid.Columns.ToList())
            {
                if ((string)(col.Header) == state)
                {
                    DivRegGrid.Columns.Remove(col);
                    break;
                }
            }
        }

        private void DivGridPopulate(ConfigCell cell, EntityRepository er)
        {
            //Clear the grids
            DivRegGrid.ItemsSource = null;
            DivRegGrid.Columns.Clear();
            ...
            //if cell does not have a div scheme, return
            if (cell.div_driver == null)
            {
                return;
            }
            ...
            DivRegGrid.ItemsSource = cell.div_driver.DriverElements;
            DivRegGrid.CanUserAddRows = false;
            DivRegGrid.CanUserDeleteRows = false;

            int i = 0;
            foreach (string s in cell.div_driver.states)
            {
                //SET UP COLUMN HEADINGS
                DataGridTemplateColumn col2 = new DataGridTemplateColumn();
                DivRegGrid.Columns.Add(CreateDivRegColumn(er, cell, s));
                i++;
            }

            DivRegGrid.ItemContainerGenerator.StatusChanged += new EventHandler(DivRegItemContainerGenerator_StatusChanged);
        }
