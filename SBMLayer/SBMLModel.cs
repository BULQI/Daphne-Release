...
namespace SBMLayer
{
    public class SBMLModel
    {
        ...
        public void ConvertDaphneToSBML()
        {
            ...
            SetPaths(Uri.UnescapeDataString(new Uri(SetUpDirectory(appPath + @"\SBML\")).LocalPath), protocol.experiment_name.Replace(".", string.Empty));
            ...
            AddCompartment("ECS", "ECS", true, ecsVol, protocol.scenario.environment.NumGridPts.Length, "");
            ...
            foreach (CellPopulation cellPop in protocol.scenario.cellpopulations)
            {
                ...
                //Membrane
                configCell = protocol.entity_repository.cells_dict[cellPop.cell_guid_ref];
                ...
                for (int comp = 0; comp < 2; comp++)
                {
                    ...
                    foreach (ConfigMolecularPopulation cmp in configComp[comp].molpops)
                    {
                        ...
                        if (comp == 0)
                        {
                            ...
                            //configurator.Protocol.entity_repository.cells[0].locomotor_mol_guid_ref
                        }
                        else
                        {
                ...
                foreach (string reaction in configCell.cytosol.reactions_guid_ref)
                {
                    ...
                    cytosolId = string.Concat("Cytosol_", configCell.CellName);
                    AddSBMLReactions(cr, cr.rate_const, protocol.entity_repository.reaction_templates_dict[cr.reaction_template_guid_ref].reac_type, cytosolId, cellPop.cell_guid_ref);
                }
        ...
        /// <summary>
        /// Populates Protocoluration object with SBML model 
        /// </summary>
        /// <param name="sc"></param>
        private void PopulateProtocol()
        {
            ...
            if (IsReactionComplex())
            {
                ...
                for (int i = 0; i < speciesList.size(); i++)
                {
                    ...
                    {
                        //configurator.Protocol.rc_scenario.environment.ecs.molpops.Add(PreparePopulation(speciesList.get(i), MoleculeLocation.Bulk, true));
